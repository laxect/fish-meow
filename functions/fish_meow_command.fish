    ...
    for command in $argv
        ...
        switch $command
            case {move,extend}_char_left
                ...
                __fish_meow_extend_by_command $command
            case {move,extend}_char_right
                commandline -C (math (commandline -C) + $count)
                __fish_meow_extend_by_command $command

            case char_up
                __fish_meow_char_up $fish_bind_mode $count
            case char_down
                __fish_meow_char_down $fish_bind_mode $count
            case extend_char_up
                __fish_meow_char_up visual $count
            case extend_char_down
                __fish_meow_char_down visual $count
            ...
            '((?:[[:alnum:]_]+|[^[:alnum:]_\\s]+|)[^\\S\\n]*)' \
                ...
                __fish_meow_next_word $fish_bind_mode $count $regex
            ...
            (?: \\n+.? |
                (?<=[^[:alnum:]_])[[:alnum:]_] |
                (?<=[[:alnum:]_\\s])[^[:alnum:]_\\s] | ) '
                __fish_meow_prev_word $fish_bind_mode $count $regex
            ...
            case till_next_char
                __fish_meow_find_char $fish_bind_mode $count forward-jump-till
            case find_next_char
                __fish_meow_find_char $fish_bind_mode $count forward-jump
            case till_prev_char
                __fish_meow_find_char $fish_bind_mode $count backward-jump-till
            case find_prev_char
                __fish_meow_find_char $fish_bind_mode $count backward-jump

            case till_next_cr
                __fish_meow_find_next_cr $fish_bind_mode $count 2
            case find_next_cr
                __fish_meow_find_next_cr $fish_bind_mode $count 1
            case till_prev_cr
                __fish_meow_find_prev_cr $fish_bind_mode $count 1
            case find_prev_cr
                __fish_meow_find_prev_cr $fish_bind_mode $count 0

            case goto_line_start
                commandline -f beginning-of-line
                __fish_meow_extend_by_mode
            case goto_line_end
                __fish_meow_goto_line_end
                __fish_meow_extend_by_mode
            case goto_first_nonwhitespace
                __fish_meow_goto_first_nonwhitespace
                __fish_meow_extend_by_mode

            case goto_file_start
                __fish_meow_goto_line $count
            case goto_line
                if test "$count_defined" = 0 # if count was provided (status 0 = success)
                    __fish_meow_goto_line $count
                end
            case goto_last_line
            ...
            case delete_selection_noyank
                __fish_meow_delete_selection

            case yank
                __fish_meow_yank
            case paste_before
                __fish_meow_paste_before "commandline -f yank"
            case paste_after
                __fish_meow_paste_after "commandline -f yank"
            case replace_selection
                __fish_meow_replace_selection "$fish_killring[1]" true

            case paste_before_clip
                __fish_meow_paste_before fish_clipboard_paste
            case paste_after_clip
                __fish_meow_paste_after fish_clipboard_paste --clip
            case replace_selection_clip
                __fish_meow_replace_selection "" fish_clipboard_paste --clip
            ...
            case select_line
                ...
                # Move cursor back if it's at the end of the line to include the last char in some contexts? 
                # Actually, Meow 'x' selects the line.
                # In fish, end-of-line puts cursor AFTER the last char usually if it's inclusive.
                # Let's stick to simple selection for now.

            case reverse_selection
                commandline -f swap-selection-start-stop

            case backward_delete
                commandline -f backward-delete-char

            case beginning_of_thing
                commandline -f beginning-of-line
                __fish_meow_extend_by_mode
            ...
            case bounds_of_thing
                commandline -f beginning-of-line begin-selection end-of-line

            case '*'
                echo "[fish-meow]" Unknown command $command >&2
        end
    end
end

function __fish_meow_extend_by_command -a piece
    if not string match -qr extend_ $piece
        commandline -f begin-selection
    end
end

function __fish_meow_extend_by_mode
    if test $fish_bind_mode = default
        commandline -f begin-selection
    end
end

function __fish_meow_find_char -a mode count fish_cmdline
...
function __fish_meow_find_next_cr -a mode count skip
...
function __fish_meow_find_prev_cr -a mode count skip
    set -l cursor (commandline -C)
...
function __fish_meow_goto_line_end
...
function __fish_meow_goto_first_nonwhitespace
...
function __fish_meow_goto_line -a number
...
function __fish_meow_char_up -a mode count
...
function __fish_meow_char_down -a mode count
    if commandline --paging-mode && not commandline --search-mode
        for i in (seq 1 $count)
            commandline -f down-line
        end
...
function __fish_meow_next_word -a mode count regex
...
function __fish_meow_prev_word -a mode count regex
...
function __fish_meow_delete_selection
...
function __fish_meow_paste_before -a cmd_paste
...
function __fish_meow_paste_after -a cmd_paste
    set -l cmd_paste $(string split " " $cmd_paste)
    set -l cursor (commandline -C)
    set -l start (commandline -B)
    set -l end (commandline -E)
...
function __fish_meow_replace_selection -a replacement cmd_paste
    set -l cmd_paste $(string split " " $cmd_paste)
